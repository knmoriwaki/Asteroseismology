...
python main.py --gpu_id 1 --isTrain --data_dir $_data_dir --ndata $_ndata --model_dir_load $model_dir --model_dir_save $_model_dir --model ${model} --fname_norm $fname_norm --input_id $_input_id --seq_length $seq_length --hidden_dim $hidden_dim --n_layer $n_layer --r_drop $_r_drop --batch_size $_batch_size --epoch $_epoch --epoch_decay $_epoch_decay --lr $_lr --loss $loss --output_dim $output_dim --output_id 13 5 --nrea_noise $_nrea_noise > ./tmp/out_${tl_name}_${today}_${model_base}.log
