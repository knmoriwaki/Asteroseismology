...
parser.add_argument("--output_dim", dest="output_dim", type=int, default=256, help="the output dimension")
...
parser.add_argument("--loss", dest="loss", default="l1_norm", help="loss function")
    ...
    with torch.no_grad():
        ...
        with open(fname, "w") as f:
            for iclass in range(args.output_dim):
                print(xmin + dx*(iclass+0.5), output[0,iclass].item(), file=f)
        print(f"# output {fname}", file=sys.stderr)
